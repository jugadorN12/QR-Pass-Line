...
const demoSellers: Seller[] = [
  { id: 'demo-lucia', name: 'Lucía Eventos', email: 'lucia@qrpassline.com', initials: 'L', demo: true },
  { id: 'demo-marcos', name: 'Marcos Fiestas', email: 'marcos@qrpassline.com', initials: 'M', demo: true },
]
...
export function SellersPage() {
  ...
  const [invitedSellers, setInvitedSellers] = useState<Seller[]>([])
  const realSellers: Seller[] = users.filter((user) => user.role === 'vendedor').map((user) => ({
    id: user.id,
    name: user.name,
    email: user.email,
    initials: user.name.slice(0, 1).toUpperCase(),
  }))
  const sellers = realSellers.length ? [...realSellers, ...invitedSellers] : [...demoSellers, ...invitedSellers]

  function inviteSeller(event: React.FormEvent<HTMLFormElement>) {
    ...
    setInvitedSellers((current) => [...current, {
      id: `invited-${crypto.randomUUID().slice(0, 8)}`,
      name: 'Vendedor invitado',
      email,
    ...
    <div className="staff-page">
      ...
      <div className="staff-layout">
        ...
        <main className="sellers-main">
          ...
          <section className="sellers-panel">
            <div className="sellers-panel-heading"><strong>Vendedores sin grupo</strong><span>⌃</span></div>
            <div className="seller-grid">
              {sellers.map((seller) => (
                <article className="seller-card" key={seller.id}>
                  <div className={`seller-avatar ${seller.demo ? 'seller-avatar-demo' : ''}`}>{seller.initials}</div>
                  ...
                  <div className="seller-actions">
                    ...
                    <button type="button" aria-label={`Ventas de ${seller.name}`}>♧ <b>{seller.demo ? 0 : 0}</b></button>
      ...
      {inviteOpen ? (
        <div className="invite-backdrop" role="dialog" aria-modal="true" aria-labelledby="invite-title">
          ...
          <form className="invite-dialog" onSubmit={inviteSeller}>
            ...
            <p>Ingresá el email del nuevo vendedor.</p>
