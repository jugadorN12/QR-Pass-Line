    ...
    <div className="staff-page">
      ...
      <div className="staff-layout">
        <aside className="staff-sidebar">
          ...
          <Link className="active" to="/vendedores" title="Vendedores">
        ...
        <main className="sellers-main">
          <div className="sellers-titlebar">
            ...
            <h1 className="doors-title">Vendedores</h1>
            <button
          ...
          <section className="sellers-panel doors-panel">
            ...
            >
              <strong>Vendedores sin grupo</strong>
            ...
            {panelOpen && (
              <div className="seller-grid doors-seller-grid">
                {sellers.map((seller) => {
                  const sellerLimits = limitations.filter((l) => l.personId === seller.id)
                  const totalQuota = sellerLimits.reduce((acc, curr) => acc + (Number(curr.quantity) || 0), 0)
                  ...
                  return (
                    <article className="seller-card doors-seller-card" key={seller.id}>
                      ...
                      <div className="seller-actions doors-actions">
                        {/* 1. Ticket Action Button: Navigates to Limitations */}
                        <Link
                          className="doors-ticket-btn"
                          to={`/vendedores/${seller.id}/limitaciones`}
                          aria-label={`Limitaciones de ${seller.name}`}
                          title="Ver limitaciones"
                        >
                          <svg width="24" height="16" viewBox="0 0 24 16" fill="none" stroke="#1e3a8a" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                            <rect x="1.5" y="1.5" width="21" height="13" rx="3" />
                            <rect x="6" y="4.5" width="12" height="7" rx="1.5" />
                          </svg>
                        </Link>

                        {/* 2. Quota / Emission count button */}
                        <button
                          type="button"
                          className="doors-quota-btn"
                          aria-label={`Cupos de ${seller.name}`}
                          onClick={() => navigate(`/vendedores/${seller.id}/limitaciones`)}
                          title={`Total cupos asignados: ${totalQuota}`}
                        >
                        ...
                        {/* 3. Delete Seller button */}
                        <button
                          type="button"
                          className="doors-trash-btn"
        ...
        <div className="doors-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="invite-title" onClick={() => setInviteOpen(false)}>
          <div className="doors-invite-dialog" onClick={(e) => e.stopPropagation()}>
            <div className="doors-invite-header">
              <strong id="invite-title">Nuevo Vendedor</strong>
              <button type="button" className="doors-invite-close" aria-label="Cerrar" onClick={() => setInviteOpen(false)}>×</button>
            </div>
            <p className="doors-invite-sub">Introduzca el correo electrónico del nuevo vendedor.</p>
            <form onSubmit={(e) => void inviteSeller(e)} className="doors-invite-form">
              <input
                autoFocus
                type="email"
                ...
                placeholder="Ingresa el correo electrónico del nuevo vendedor"
                required
              />
