...
export function QrTypesPage() {
  ...
  return (
    <div className="staff-page qr-page">
      ...
      {preview ? <div className="qr-preview-backdrop" role="dialog" aria-modal="true"><button className="time-picker-dismiss" type="button" aria-label="Cerrar vista previa" onClick={() => setPreview(null)} /><section className="qr-preview-card" style={preview.backgroundImage ? { backgroundImage: `linear-gradient(rgba(255,255,255,.88),rgba(255,255,255,.96)), url(${preview.backgroundImage})` } : undefined}>      <div className="qr-preview-mark"><img src={localStorage.getItem('qr-pass-line.logo') || '/app-icon.png'} alt="" /></div><p className="kicker">CONSUMIBLE_{preview.kind.toUpperCase()}</p><h2>{preview.name}</h2><p className="muted">{preview.description}</p><div className="qr-preview-code">QR PASS LINE</div><button className="btn btn-primary btn-block" type="button" onClick={() => setPreview(null)}>Cerrar</button></section></div> : null}
    </div>
  )
}
